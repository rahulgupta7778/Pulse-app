const Connections = {
  ...
  async render() {
    ...
    // Built-in services
    const builtins = [
      { key: 'googleCalendar', icon: '📅', name: 'Google Calendar' },
      { key: 'gmail', icon: '📧', name: 'Gmail' }
    ];

    // Dynamic connectors from meta
    const dynamic = Object.keys(meta).map(key => ({
      key,
      icon: meta[key].icon,
      name: meta[key].name,
      desc: meta[key].desc
    ...
    const allServices = [...builtins, ...dynamic];

    list.innerHTML = allServices.map(svc => {
      const status = s[svc.key];
