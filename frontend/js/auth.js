const Auth = {
  ...
  setupLanding() {
    // Back to home link on Auth page
    const backBtn = document.getElementById('backToHomeLink');
    if (backBtn) {
    ...
    // Scroll to features
    const exploreBtn = document.getElementById('landingBtnExploreFeatures');
    if (exploreBtn) {
      exploreBtn.addEventListener('click', () => {
        document.getElementById('playgroundSection')?.scrollIntoView({ behavior: 'smooth' });
      });
    }

    // Log In trigger from header
    const loginHeaderBtn = document.getElementById('landingBtnLoginHeader');
    if (loginHeaderBtn) {
    ...
    // Sign Up trigger from header and hero
    const signupBtns = [
      document.getElementById('landingBtnSignupHeader'),
      document.getElementById('landingBtnGetStartedHero'),
      document.getElementById('landingBtnGetStartedCta')
    ];
  ...
  logout() {
    Store.clearUser();
    if (typeof Reminders !== 'undefined') Reminders.stopPolling();
  ...
  async showApp() {
    ...
    try {
      ...
      this.setupProfile();
      if (typeof Reminders !== 'undefined') Reminders.startPolling();
      if (typeof Connections !== 'undefined') Connections.render();
      if (typeof Mood !== 'undefined') Mood.loadToday();
