...
const Analytics = {
  ...
  pomodoro: {
    ...
    async init() {
      ...
      await this.loadTasksDropdown();
      await this.renderHistory();
    },

    async loadTasksDropdown() {
      const select = document.getElementById('pomoTaskSelect');
      if (!select) return;
      try {
        const tasks = await Store.getTasks();
        const activeTasks = tasks.filter(t => !t.completed);
        select.innerHTML = '<option value="">-- No Linked Task --</option>' + activeTasks.map(t => {
          return `<option value="${t.id}">${Utils.escapeHtml(t.title)}</option>`;
        }).join('');
      } catch {}
    },

    async renderHistory() {
      const listEl = document.getElementById('pomoSessionsList');
      if (!listEl) return;
      try {
        const sessions = await Store.getFocusSessions();
        const tasks = await Store.getTasks();
        if (sessions.length === 0) {
          listEl.innerHTML = '<p style="font-size: 0.8rem; color: var(--text-muted); margin: 0;">No focus sessions logged yet. Let\'s start your first session!</p>';
          return;
        }
        listEl.innerHTML = sessions.slice(0, 10).map(s => {
          const t = tasks.find(x => x.id === s.taskId);
          const taskLabel = t ? `linked to "${Utils.escapeHtml(t.title)}"` : 'standalone session';
          const timeLabel = new Date(s.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
          const dateLabel = new Date(s.createdAt).toLocaleDateString([], { month: 'short', day: 'numeric' });
          const moodEmoji = s.moodBefore === 'stressed' ? '🧘' : s.moodBefore === 'tired' ? '☕' : s.moodBefore === 'energetic' ? '🚀' : '🎯';
          return `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.5rem; background: rgba(255, 255, 255, 0.02); border-radius: 8px; border: 1px solid var(--border); font-size: 0.8rem;">
              <div style="display: flex; align-items: center; gap: 0.5rem;">
                <span style="font-size: 1rem;">${moodEmoji}</span>
                <div>
                  <strong style="color: var(--text);">${s.duration} min session</strong>
                  <div style="color: var(--text-muted); font-size: 0.72rem;">${taskLabel}</div>
                </div>
              </div>
              <div style="color: var(--text-muted); text-align: right; font-size: 0.72rem;">
                <div>${timeLabel}</div>
                <div>${dateLabel}</div>
              </div>
            </div>
    ...
    async complete() {
      ...
      const pauseBtn = document.getElementById('pomoPauseBtn');
      const status = document.getElementById('pomoStatus');
      const sessions = document.getElementById('pomoSessions');
      if (startBtn) startBtn.disabled = false;
      if (pauseBtn) pauseBtn.disabled = true;
      if (status) status.textContent = 'Great focus session! Take a 5-min break.';
      if (sessions) sessions.textContent = `Sessions completed: ${this.sessionCount}`;
      ...
      try {
        ...
        // Fetch today's mood log if available, else default to 'focused'
        let moodBefore = 'focused';
        try {
          const logs = await fetch('/api/analytics/load-score', {
            headers: { 'Authorization': 'Bearer ' + localStorage.getItem('pulse_token') }
          });
          if (logs.ok) {
            const l = await logs.json();
            moodBefore = l.mood;
          }
        } catch {}
        ...
        Toast.show("🎉 Focus session saved to cloud!", "success");
        await this.renderHistory();
      } catch (err) {
